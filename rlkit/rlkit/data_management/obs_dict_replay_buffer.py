...
class ObsDictRelabelingBuffer(ReplayBuffer):
    ...
    def end_epoch(self, epoch):
        if self.save_buffer and epoch % logger.get_snapshot_gap() == 0:
            rb_dict = {}
            rb_dict['_actions'] = self._actions
            rb_dict['_terminals'] = self._terminals
            for key in self.ob_keys_to_save + self.internal_keys:
                rb_dict['_obs/'+key] = self._obs[key]
                rb_dict['_next_obs/'+key] = self._next_obs[key]
            rb_dict['_idx_to_future_obs_idx'] = self._idx_to_future_obs_idx
            rb_dict['_top'] = self._top
            rb_dict['_size'] = self._size
            pickle.dump(rb_dict, open(logger.get_snapshot_dir()+'/replay_buffer.pkl', "wb"))

    def load(self, rb_dict_path, size=None):
        rb_dict = pickle.load(open(rb_dict_path, "rb"))
        if size is None:
            size = self.max_size
        self._actions[:size, :] = rb_dict['_actions'][:size, :]
        self._terminals[:size, :] = rb_dict['_terminals'][:size, :]
        for key in self.ob_keys_to_save + self.internal_keys:
            self._obs[key][:size, :] = rb_dict['_obs/'+key][:size, :]
            self._next_obs[key][:size, :] = rb_dict['_next_obs/'+key][:size, :]
        self._idx_to_future_obs_idx[:size] = rb_dict['_idx_to_future_obs_idx'][:size]
        self._top = min(rb_dict['_top'], size)
        self._size = min(rb_dict['_size'], size)
